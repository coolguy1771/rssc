...
var _ = Describe("AutoscaleSet Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind AutoscaleSet")
			err := k8sClient.Get(ctx, typeNamespacedName, autoscaleset)
			if err != nil && errors.IsNotFound(err) {
				resource := &scalesetv1alpha1.AutoscaleSet{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance AutoscaleSet")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &AutoscaleSetReconciler{
				Client: k8sClient,
